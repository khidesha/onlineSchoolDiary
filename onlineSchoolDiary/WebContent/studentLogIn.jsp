...
//		}
		String tmp = new SimpleDateFormat("dd/MM/yyyy").format(Calendar.getInstance().getTime());
	...
	<tr>
		...
		out.println(new SimpleDateFormat("dd/MM/yyyy").format(c.getTime()));%> </td>
		<td> <%c.set(Calendar.DAY_OF_WEEK, Calendar.TUESDAY);
		out.println(new SimpleDateFormat("dd/MM/yyyy").format(c.getTime()));%> </td>
		<td> <%c.set(Calendar.DAY_OF_WEEK, Calendar.WEDNESDAY);
		out.println(new SimpleDateFormat("dd/MM/yyyy").format(c.getTime()));%> </td>
		<td> <%c.set(Calendar.DAY_OF_WEEK, Calendar.THURSDAY);
		out.println(new SimpleDateFormat("dd/MM/yyyy").format(c.getTime()));%> </td>
		<td> <%c.set(Calendar.DAY_OF_WEEK, Calendar.FRIDAY);
		out.println(new SimpleDateFormat("dd/MM/yyyy").format(c.getTime()));%> </td>
	</tr>
	<% for (int i=0; i<arr.size(); i++){ %>
		<tr>
			
			<td><% out.println(arr.get(i).subject_name); %> </td>
			<% for (int j=0; j<5; j++){ %>
				<td> </td>
				<%
				}%>
