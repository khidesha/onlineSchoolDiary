...
<body>
	<form action="/onlineSchoolDiary/editSubject" method="post">
		საგანი: <br> <select class="select" name="subject">
			<%
			...
			%>
		</select> <br> საგნის სახელი:<input class="input" type="text"
			name="subjectName"> მასწავლებელი: <br> <select
			class="select" name="teacherID">
			<%
				ArrayList<Teacher> arr2 = sm.getTeachers(schoolId);
				for (int i = 0; i < arr2.size(); i++) {
					out.println("<option value=" + arr2.get(i).getUserId() + ">"
							+ arr2.get(i).getUserId() + " "
							+ arr2.get(i).getUsername() + "</option>");
				}
			%>
		</select> <br> კლასი: <br> <select class="select" name="groupID">
			<%
